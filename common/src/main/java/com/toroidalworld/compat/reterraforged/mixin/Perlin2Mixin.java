...
@Mixin(value = Perlin2.class, remap = false)
public abstract class Perlin2Mixin {
    @Shadow
    @Final
    private int seed;
    ...
    private float toroidal$closeOnLap(float x, float z, int seed, Operation<Float> original) {
        ...
        float sum = RtfOctaves.sum(frame, Perlin2::sample, this.interpolation, x, z, this.seed, this.octaves,
                this.frequency, this.lacunarity, this.gain, this.gain);
