...
@Mixin(targets = "raccoonman.reterraforged.world.worldgen.noise.module.PerlinRidge", remap = false)
public abstract class PerlinRidgeMixin {
    @Shadow
    @Final
    ...
    private float toroidal$closeOnLap(float x, float z, int seed, Operation<Float> original) {
        ...
        float value = RtfOctaves.ridge(frame, PerlinSample::sample, this.interpolation, x, z, seed, this.octaves,
                this.frequency, this.lacunarity, this.gain, this.spectralWeights);
